...
(init-message "Emacs version : %s" emacs-version)
(init-message "window-system : %s" window-system)
(init-message "init file     : %s" user-init-file)
(init-message "running on    : %s" system-name)
(init-message "system type   : %s" system-type)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :exports none
(skj/feature-start 'compat)
#+END_SRC

This is required by a number of packages and so it's worth getting it installed here. Also, this should not
use =use-package= for some reason.
...
(use-package use-package-ensure-system-package
  :ensure nil ;; now a part of use-package
  :if (skj/current-feature-required-p)
...
(if (image-type-available-p 'imagemagick)
    (init-message "Emacs has imagemagick support :)")
  (init-message "Emacs does not have imagemagick support :("))
#+END_SRC
...
(when (and (skj/current-feature-required-p) window-system)
  ...
  (use-package nerd-icons
    ...
    :demand t
    :custom
    ;; Note that this overrides the default "Symbols Nerd Font Mono"
...
For those things that don't know about nerd fonts, and continue to use the all-the-icons package, here is a bridge. Note
the need to install the resource fonts;
...
(use-package multiple-cursors
  :if (skj/current-feature-required-p)
  :bind (;; When you have an active region that spans multiple lines,
         ;; the following will add a cursor to each line:
         ("M-n e l"       . mc/edit-lines)
         ("M-n e a"       . mc/edit-beginnings-of-lines)
         ("M-n e e"       . mc/edit-ends-of-lines)
         ;; When you want to add multiple cursors not based on continuous
         ;; lines, but based on keywords in the buffer, use:
         ("C-M-<mouse-1>" . mc/toggle-cursor-on-click)

         ("M-n m <down>"  . mc/mark-next-lines)
         ("M-n m n"       . mc/mark-next-like-this)
         ("M-n m f"       . mc/mark-next-word-like-this)

         ("M-n m <up>"    . mc/mark-previous-lines)
         ("M-n m p"       . mc/mark-previous-like-this)
         ("M-n m b"       . mc/mark-previous-word-like-this)
         
         ("M-n m m"       . mc/mark-more-like-this)
         ("M-n m a"       . mc/mark-all-like-this)
         ("M-n m w"       . mc/mark-all-words-like-this)
         ("M-n m r"       . mc/mark-all-in-region)

         ("M-n SPC"       . set-rectangular-region-anchor)))
#+END_SRC
...
(use-package mc-extras
  ...
  :bind (("M-n m s n" . mc/mark-next-sexps)
         ("M-n m s p" . mc/mark-previous-sexps)
         ("M-n m <"   . mc/mark-all-above)
         ("M-n m >"   . mc/mark-all-below)
         ("M-n r m"   . mc/remove-current-cursor)
         ("M-n r e"   . mc/remove-cursors-at-eol)
         ("M-n r d"   . mc/remove-duplicated-cursors)
         ("M-n |"     . mc/move-to-column)
         ("M-n ~"     . mc/compare-chars)
         ("M-n f f"   . mc/freeze-fake-cursors)
         ("M-n f u"   . mc/unfreeze-fake-cursors)
         ("M-n f d"   . mc/freeze-fake-cursors-dwim)))
#+END_SRC
...
(when (skj/current-feature-required-p)
  ...
  (pretty-hydra-define hydra-mc
    ...
    ("Edit"
     (("e l"   mc/edit-lines                 "Edit lines")
      ("e a"   mc/edit-beginnings-of-lines   "Edit at start of lines")
      ("e e"   mc/edit-ends-of-lines         "Edit at end of lines"))
     "Mark"
     (("m <down>"
               mc/mark-next-lines            "Next lines")
      ("m n"   mc/mark-next-like-this        "Next like this")
      ("m f"   mc/mark-next-word-like-this   "Next word like this")
      ("m s n" mc/mark-next-sexps            "Next s-expressions")
      ("m >"   mc/mark-all-below             "All below")

      ("m <up>"
               mc/mark-previous-lines        "Previous lines")
      ("m p"   mc/mark-previous-like-this    "Previous like this")
      ("m b"   mc/mark-previous-word-like-this "Previous word like this")
      ("m s p" mc/mark-previous-sexps        "Previous s-expressions")
      ("m <"   mc/mark-all-above             "All above")
      
      ("m m"   mc/mark-more-like-this        "More like this")
      ("m a"   mc/mark-all-like-this         "All like this")
      ("m w"   mc/mark-all-words-like-this   "All words like this"))
     "Remove"
     (("r m"   mc/remove-current-cursor      "Remove current cursor")
      ("r a"   mc/remove-cursors-at-bol      "Remove cursors at SOL")
      ("r e"   mc/remove-cursors-at-eol      "Remove cursors at EOL")
      ("r d"   mc/remove-duplicated-cursors  "Remove duplicate cursors")
      ("r b"   mc/remove-cursors-on-blank-lines  "Remove cursors on blank lines"))
     "Special"
     (("SPC"   set-rectangular-region-anchor "Set rectangular anchor")
      ...
      ("|"     mc/move-to-column             "Move to column")
      ("f f"   mc/freeze-fake-cursors        "Freeze cursors")
      ("f u"   mc/unfreeze-fake-cursors      "Unfreeze cursors")
      ("f d"   mc/freeze-fake-cursors-dwim   "Toggle frozen state"))))

  (bind-key "<f9> m" 'hydra-mc/body))
#+END_SRC
...
This package comes with rich transformers for commands from ivy and counsel. It should be easy enough to define your own
transformers too.
...
This [[https://github.com/sulami/literate-calc-mode.el][package]] enables in-line display of math results in markup buffers.
...
#+BEGIN_SRC emacs-lisp :exports none
(skj/feature-end 'vterm)
#+END_SRC

*** The Emacs Shell

#+BEGIN_SRC emacs-lisp :exports none
(skj/feature-start 'eshell)
#+END_SRC

The Emacs shell ([[https://www.gnu.org/software/emacs/manual/html_node/eshell/][eshell]]) is an integrated shell, not always terminal compatible but more Emacs integrated.

Also consider:
 
- [[https://github.com/4da/eshell-toggle][eshell-toggle]]
- [[https://github.com/peterwvj/eshell-up][eshell-up]]
- [[https://github.com/iostapyshyn/eshell-vterm][eshell-vterm]]
...
This [[https://github.com/tom-tan/esh-buf-stack][library]] adds a buffer stack feature to Eshell. It is inspired by the buffer stack in Zsh. You can push the current
command to the buffer stack by using ~M-q~, and after executing another command, you can see the top of the stack popped.
...
This library adds the following help functions and support for Eshell; the =run-help= function inspired by Zsh, and eldoc
support. By using ~M-x eldoc-mode~ in Eshell, you can see help strings for the pointed command in minibuffer. And by using
~M-x esh-help-run-help~, you can see full help string for the command.
...
This [[https://github.com/xuchunyang/eshell-git-prompt][package]] provides some themes of Emacs Shell (Eshell) prompt.
...
This [[https://github.com/akreisher/eshell-syntax-highlighting][package]] adds syntax highlighting to the Emacs Eshell. It highlights user commands at the interactive prompt to
provide feedback on the validity of commands and syntax. This package is inspired by zsh-syntax-highlighting and the native highlighting of fish.
...
Package ~github-review~ lets you submit GitHub code review with Emacs. With ~github-review-start~ you can pull the content
of a pull request into a buffer as a diff with comments corresponding to the PR description. In that buffer you can add
comment (global and inline) that you want to make on the pull request. You can use ~github-review~ with forge. When your
cursor is over a pull request, you can call ~github-review-forge-pr-at-point~ to start a code review.
...
Alternatively, this stack uses the ~gh~ foundation instead of ~ghub~.
...
(when (skj/current-feature-required-p)
  ...
  (defcustom skj/babel-safe-languages '()
    "A list of languages safe for Babel evaluation."
    :tag "Babel safe languages"
    :type '(repeat string)
    :group 'skj-org)

  (defcustom skj/babel-safe-directories '()
    "A list of directories safe for Babel evaluation."
    :tag "Babel safe directories"
    :type '(repeat directory)
    :group 'skj-org)

  (defun skj/check-ob-lang (lang)
    (cond ((stringp lang) lang)
          ((symbolp lang) (symbol-name lang))
          (error "provided lang not a string or symbol (given a %s)" (type-of lang))))
  
  (defun skj/org-babel-add-safe-language (lang)
    (add-to-list 'skj/babel-safe-languages (skj/check-ob-lang lang)))

  (defun skj/add-babel-safe-directories (root &optional sub-dirs add-root)
    (when (or add-root (null sub-dirs))
        (add-to-list 'skj/babel-safe-directories root))
    (dolist (dir sub-dirs)
      (skj/add-babel-safe-directories (concat-path root dir))))
  
  (defun skj/org-confirm-babel-evaluate (lang body)
    "Return t if LANG not in `skj/babel-safe-languages'."
    (let ((lang (skj/check-ob-lang lang))
          (dir-name (file-name-parent-directory buffer-file-truename)))
      (message "Babel confirm eval of %s; is it in %s? is %s a safe directory? -- %s"
               lang skj/babel-safe-languages
               dir-name
               (and (member lang skj/babel-safe-languages)
                    (member dir-name skj/babel-safe-directories)))
...
(when (skj/current-feature-required-p)
  ...
  (use-package ob-gnuplot
    ...
    (skj/org-babel-add-safe-language 'gnuplot)))
#+END_SRC
...
(when (skj/current-feature-required-p)
  ...
  (use-package ob-dot
    ...
    (skj/org-babel-add-safe-language 'dot)))
#+END_SRC
...
(when (skj/current-feature-required-p)
  ...
  (use-package ob-plantuml
    ...
    (skj/org-babel-add-safe-language 'plantuml)))
#+END_SRC
...
(when nil ; (skj/current-feature-required-p)
  ...
  (use-package typst-ts-mode
    :ensure-system-package typst)

  ; (use-package outline-indent-mode
  ;   :after typst-ts-mode)

  (unless (executable-find "typstyle")
    (call-process "cargo install typstyle --locked"))
  
  (unless (executable-find "tinymist")
    (call-process "cargo install --git https://github.com/Myriad-Dreamin/tinymist --locked tinymist"))
...
(when (skj/current-feature-required-p)
  ...
  (use-package org-ref-ivy
    :ensure nil ;; included in org-ref, uses ivy-bibtex
    :after (org-ref ivy-bibtex)))
#+END_SRC
...
This is nice for languages still using tagging. by default on macos the ctag command is next to useless so always
install the Universal Ctags package for a way more up-to-date and configurable option.
...
(when (skj/current-feature-required-p)
  ...
  (use-package company-ctags
    :after company
    :ensure-system-package universal-ctags
    :config
    (company-ctags-auto-setup)))
#+END_SRC

Might also consider [[https://www.virgilsbbq.com/locations/times-square][ctags-update]].
...
Setup the search path for tree-sitter language libraries. This finds the dynamic libraries that hold the parsers.

- =/usr/local/lib= :: The best place for shared libraries.
- =~/.local/lib= :: The best place for your local libraries.
- =~/.tree-sitter/bin= :: An older path recommended by tree-sitter tools.
- =~/emacs.d/tree-sitter= :: The path used by some Emacs libraries.
...
(when (and (skj/current-feature-required-p)
  ...
  (use-package tree-sitter
    ...
    (add-to-list 'tree-sitter-load-path
                 (expand-file-name "/usr/local/lib"))
    (add-to-list 'tree-sitter-load-path
                 (expand-file-name "~/.local/lib"))
    (add-to-list 'tree-sitter-load-path
                 (expand-file-name "~/.tree-sitter/bin"))
    (add-to-list 'tree-sitter-load-path
                 (expand-file-name
...
https://github.com/Ziqi-Yang/.emacs.d/blob/main/languages/l-treesit.el

https://github.com/Ziqi-Yang/.emacs.d/blob/main/languages/l-languages.el
...
(when (>= emacs-version-number 29)
  (init-message "ts-fold not available for Emacs >= 29"))
#+END_SRC
...
(when (skj/current-feature-required-p)
  ...
  (use-package jq-mode
    ...
    (skj/org-babel-add-languages 'jq))
...
;;  :if (skj/current-feature-required-p)
;;  :custom (elisp-autofmt-python-bin (executable-find "python3"))
...
(when (skj/current-feature-required-p)
  ...
  (defcustom skj-dev/common-lisp-packages '(allegro-express clisp sbcl)
  ...
  (message "Slime current interpreter %s, executable %s"
           skj-dev/common-lisp-interpreter
...
(when (skj/current-feature-required-p)
  ...
  (let ((local-sdml-mode-repo (concat-path skj/project-root-dir "sdm-lang/emacs-sdml-mode")))
        (unless (file-directory-p local-sdml-mode-repo)
          (init-message "Cloning emacs-sdml-mode to local repo")
          (skj/git-clone "https://github.com/sdm-lang/emacs-sdml-mode.git"
                         local-sdml-mode-repo))))
#+END_SRC
...
(when (skj/current-feature-required-p)
  ...
  (use-package sdml-mode
    ...
    :mode "\\.sdml?\\'"
    :load-path "/Users/simonjo/Projects/sdm-lang/emacs-sdml-mode")
...
(when (skj/current-feature-required-p)
  ...
  (use-package sdml-ispell
    ...
    :load-path "/Users/simonjo/Projects/sdm-lang/emacs-sdml-ispell")
...
(when (skj/current-feature-required-p)
  ...
  (use-package flycheck-sdml
    ...
    :load-path "/Users/simonjo/Projects/sdm-lang/emacs-flycheck-sdml")
  
  (require 'flycheck-sdml))
#+END_SRC

Code folding is a nice feature as well.
...
(when (and (skj/current-feature-required-p)
  ...
  (use-package sdml-fold
    ...
    :load-path "/Users/simonjo/Projects/sdm-lang/emacs-sdml-fold")
  
  (require 'sdml-fold))
#+END_SRC

This enables SDML in org-mode Babel blocks.
...
(when (skj/current-feature-required-p)
  ...
  (use-package ob-sdml
    :defer nil
    :after (org ob sdml-mode)
    :load-path "/Users/simonjo/Projects/sdm-lang/emacs-ob-sdml"
    :init
    (skj/org-babel-add-languages 'sdml)
    (skj/org-babel-add-safe-language 'sdml))
...
- [[http://mlton.org][mlton]] -- Whole-program, optimizing compiler for Standard ML
- [https://www.polyml.org/][Poly/ML] -- The Poly/ML implementation of Standard ML
- [[https://mosml.org/][Moscow ML]] -- Light-weight implementation of Standard ML
- [[https://www.smlnj.org/][Standard ML of New Jersey]] -- Compiler for the Standard ML '97 programming language

Plus

- [[https://github.com/shwestrick/smlfmt][smlfmt]] -- Custom parser and code formatter for Standard ML
- [[https://github.com/diku-dk/smlpkg][smlpkg]] -- Package manager for Standard ML libraries and programs
...
Not currently using OCaml, although had to make some small changes to existing code at one point. If needed, the
...
- ocaml
- ocaml-findlib
- ocaml-zarith (replaces ocalm-num)
- ocamlbuild
...
#+BEGIN_SRC emacs-lisp :exports none
(skj/feature-start 'dot-net ".net, CLR, C#, F#")
#+END_SRC

A wrapper around the .NET command-line interface.
...
Support for .NET project files (csproj, vbproj).
...
(when (skj/current-feature-required-p)
  ...
  ;; The package below seems very flaky
  ;;(use-package all-the-icons-gnus
  ;;  :if window-system
  ;;  :after (all-the-icons gnus)
  ;;  :hook (gnus-mode . all-the-icons-gnus-setup))
  )
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(when (skj/current-feature-required-p)
  (let ((work-init-path (concat-path skj/org-init "work-init.org")))
    (init-message "Checking for work org-mode init file ~s" work-init-path)
    (if (file-exists-p work-init-path)
        (org-babel-load-file work-init-path)
      (init-message "Cannot file work init file: %s" work-init-path))))
#+END_SRC

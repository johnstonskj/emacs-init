...
(when (skj/current-feature-required-p)
  ...
  (exec-path-prepend (expand-file-name "~/bin")))
#+END_SRC
...
Alternatively, the following packages exist that may be useful in setting up auth stores.
...
Git support is entirely built into Emacs, so this section only adds additional tools.
  ...
  (unless (file-directory-p quicklisp-directory)
    ...
    (let ((quicklisp-source (expand-file-name
          ...
          (quicklisp-sig (expand-file-name
                          (concat-path quicklisp-directory
                                       "tmp-quicklisp.lisp.sig"))))
      (require 'url) ; built-in
      (url-copy-file "https://beta.quicklisp.org/quicklisp.lisp"
                     quicklisp-source)
      (url-copy-file "https://beta.quicklisp.org/quicklisp.lisp.sig"
                     quicklisp-sig)
      ...
      (cond
       ((eq skj-dev/common-lisp-program "clisp")
        (call-process
         (format "clisp -i %s -x '(quicklisp-quickstart:install)'"
                 quicklisp-source)))
       ((eq skj-dev/common-lisp-program "sbcl")
        (call-process
         (format "sbcl --load %s --eval '(quicklisp-quickstart:install)' --quit"
                 quicklisp-source)))
       (t
        (init-message "Could not load quicklisp for %s" skj-dev/common-lisp-program))))))
#+END_SRC
...
(when (skj/current-feature-required-p)
  ...
  (use-package slime
    :custom
    (inferior-lisp-program (executable-find skj-dev/common-lisp-program))
...
(when (skj/current-feature-required-p)
  ...
  (defcustom skj-dev/scheme-packages '(guile racket)
    "Name of Scheme implementations to enable."
    :tag "Enabled Schemes"
    :type '(set (const :tag "Chez Scheme" chez)
                (const :tag "chibi-scheme" chibi)
                (const :tag "CHICKEN Scheme" chicken)
                (const :tag "Gambit Scheme" gambit)
                (const :tag "Gauche" gauche)
                (const :tag "Guile" guile)
                (const :tag "MIT/GNU Scheme" mit)
                (const :tag "Racket" racket))
    :group 'skj-dev)

  (defcustom skj-dev/scheme-program "racket"
    "Name of Scheme program to run as default intepreter."
    :tag "Scheme intepreter"
    :type '(radio (const :tag "Chez Scheme" "scheme")
                  (const :tag "Chez Scheme (Petite)" "petite")
                  (const :tag "chibi-scheme" "chibi-scheme")
                  (const :tag "CHICKEN Scheme" "csi")
                  (const :tag "Gambit Scheme" "gsi")
                  (const :tag "Gauche" "gosh")
                  (const :tag "Guile" "guile")
                  (const :tag "MIT/GNU Scheme" "mit-scheme")
                  (const :tag "Racket" "racket"))
    :group 'skj-dev))
#+END_SRC
...
(when (skj/current-feature-required-p)
  ...
  (use-package scheme
    :ensure nil
    :custom
    (scheme-program-name  skj-dev/scheme-program))
...
(when (skj/current-feature-required-p)
  ...
  (when (and (member 'chez skj-dev/scheme-packages)
             (not (system-mac-p)))
    (use-package geiser-chez
      :ensure-system-package chezscheme
      :after geiser))

  (when (member 'chibi skj-dev/scheme-packages)
    (use-package geiser-chibi
      :ensure-system-package chibi-scheme
      :after geiser))

  (when (member 'chicken skj-dev/scheme-packages)
    (use-package geiser-chicken
      :ensure-system-package chicken
      :after geiser))

  (when (member 'gambit skj-dev/scheme-packages)
    (use-package geiser-gambit
      :ensure-system-package gambit-scheme
      :after geiser))

  (when (member 'gauche skj-dev/scheme-packages)
    (use-package geiser-gauche
      :ensure-system-package gauche
      :after geiser))

  (when (member 'guile skj-dev/scheme-packages)
    (use-package geiser-guile
      :ensure-system-package guile
      :after geiser))

  (when (member 'mit skj-dev/scheme-packages)
    (use-package geiser-mit
      :ensure-system-package mit-scheme
      :after geiser))
  
  (when (member 'racket skj-dev/scheme-packages)
    (use-package geiser-racket
      :after (geiser racket-mode))))
#+END_SRC
...
(when (skj/current-feature-required-p)
  ...
  (defun rustic-mode-auto-save-hook ()
    ...
    (init-message "rustic-mode-auto-save-hook")
    (when buffer-file-name
      (setq-local compilation-ask-about-save nil)))

  (defun rustic-mode-minor-modes ()
    (init-message "rustic-mode-minor-modes")

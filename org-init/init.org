...
(when (and (skj/current-feature-required-p)
  ...
  (let ((git-url "https://github.com/emacs-tree-sitter/ts-fold.git")
        (local-dir "/Users/simonjo/Projects/ts-fold"))
    (when (not (file-directory-p local-dir))
      (call-process (format "git clone %s %s" git-url local-dir))))
...
Git support is built into Emacs as =vc-git=, so this section only adds additional tools.
  ...
  (defun skj-dev/common-lisp-executable ()
    (let* ((name skj-dev/common-lisp-interpreter)
      ...
      (unless (null executable)
        (executable-find (cdr executable))))))
#+END_SRC
...
(when (skj/current-feature-required-p)
  ...
  (skj-dev/common-lisp-run-script "file.lisp" "(quicklisp-quickstart:install)")

  (let ((quicklisp-directory (expand-file-name "~/quicklisp")))
...
(when (skj/current-feature-required-p)
  ...
  (let ((git-url "https://github.com/sdm-lang/emacs-sdml-mode.git")
        (local-dir "/Users/simonjo/Projects/sdml-lang/emacs-sdml-mode"))
    (when (not (file-directory-p local-dir))
      (call-process (format "git clone %s %s" git-url local-dir)))))
#+END_SRC

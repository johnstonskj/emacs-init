...
This [[https://github.com/kickingvegas/casual-calc][package]] provides an opinionated Transient-based user interface to support the casual usage of Emacs Calc.
...
(when (skj/current-feature-required-p)
  ...
  (defun skj/org-confirm-babel-evaluate (lang body)
    ...
    (let ((lang (skj/check-ob-lang lang)))
      (message "Babel confirm eval of %s; is it in %s? -- %s"
               lang skj/babel-safe-languages
               (member lang skj/babel-safe-languages))

...
Now we can redefine ~init-message~ to include the current feature stack.
...
Now we get to the main components
...
/Note/: The persisted state is saved under =user-emacs-directory/.cache/treemacs-persist= by default. The exact file
location is saved in the variable ~treemacs-persist-file~.
